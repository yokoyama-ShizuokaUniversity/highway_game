...
- 各地点の下にある入力枠へ漢字で名称を入力します。末尾の「IC」「JCT」「PA」「SA」「SIC」を省略した名称でも正解になります。正解するとアイコンが緑色に、両側が正解になると該当区間が緑色（#00b06b）で開通します。入力枠は線の向きに合わせて少しずらしているため、重なりづらくなっています。
...
- ノード定義: `id`（ユニーク ID）、`name`、`kind`（IC/JCT/PA/SA/SIC）、`km`（任意の距離情報）、`lat` `lon`（緯度経度、推奨）、`x` `y`（0〜100 の相対座標の上書き用）。名前末尾の「IC/JCT/PA/SA/SIC」を入力時は省略可能です。
